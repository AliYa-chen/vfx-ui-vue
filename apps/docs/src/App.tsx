import { useEffect, useState } from "vfx-ui-vue/compat";
...
function ShaderCapturePage() {
  ...
  return (
    <main class="capture-shell" aria-label={`${shader.label} preview capture`}>
      <div class={`capture-preview preview shader-preview ${shader.id} ${shader.runtime === "dom" ? "is-dom-preview" : ""} ${shader.category === "Footers" ? "is-footer-preview" : ""} ${shader.category === "Glass" || (shader.id === "radiant-dots" || shader.id === "astra-field") ? "is-optical-preview" : ""}`} data-variant={variant?.id}>
        <Suspense v-slots={{
          default: () => Preview ? <Preview {...captureProps} interactive={false} style={shader.id === "spectral-card" ? { width: 460, height: 560 } : undefined} /> : null,
          fallback: () => <div class="preview-loading" role="status">Loading renderer…</div>,
        }} />
        <span class="capture-ready" data-capture-ready={shader.id} aria-hidden="true" />
      </div>
    </main>
  );
}

function VfxUiVueApp() {
  ...
  return (
    <>
      <header class="topbar">
        <button class="icon-btn" aria-label="Open navigation" onClick={() => setSidebarOpen(true)}>
          <MenuIcon />
        </button>
        <button class="topbar-brand-button" aria-label="vfx-ui-vue home" onClick={selectHome}>
          <BrandMark compact />
        </button>
      ...
      <div class="app">
        <Sidebar
          ...
          onTheme={selectTheme}
        />
        {sidebarOpen && <button class="mobile-nav-scrim" aria-label="Close navigation" onClick={() => setSidebarOpen(false)} />}
        <div class="pane">
          <div class="pane-scroll scroll-area">
            {page === "browse" ? (
              <ErrorBoundary>
                <BrowsePage
              ...
              <main class="browse-page" aria-labelledby="not-found-title">
                <header class="browse-header">
                  <h1 id="not-found-title">Page not found</h1>
                  <p class="lede">This vfx-ui-vue URL does not match a component, variant, or documentation page.</p>
...
export default function App() {
  return captureFromUrl() ? <ShaderCapturePage /> : <VfxUiVueApp />;
}
