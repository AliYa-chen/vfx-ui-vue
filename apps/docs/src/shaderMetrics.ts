...
export const VFXUIVUE_METRICS_STORAGE_KEY = "vfx-ui-vue-metrics-v1";
...
function readStore(): ShaderMetricStore {
  try {
    const stored = localStorage.getItem(VFXUIVUE_METRICS_STORAGE_KEY);
...
function incrementShaderMetric(id: ReadyShader["id"], metric: keyof ShaderMetrics) {
  ...
  store[id] = next;
  try {
    localStorage.setItem(VFXUIVUE_METRICS_STORAGE_KEY, JSON.stringify(store));
  } catch {
