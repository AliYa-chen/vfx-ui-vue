import { useMemo } from "vfx-ui-vue/compat";

type SyntaxLanguage = "css" | "glsl" | "javascript" | "jsx" | "markdown" | "shader-javascript" | "text" | "tsx" | "typescript" | "vue";
...
function normalizedLanguage(language: string): SyntaxLanguage {
  if (["css", "glsl", "javascript", "jsx", "markdown", "shader-javascript", "text", "tsx", "typescript", "vue"].includes(language)) {
...
export function tokenizeSyntax(code: string, requestedLanguage: string) {
  ...
  const tokens: SyntaxToken[] = [];
  const isJavaScript = ["javascript", "jsx", "shader-javascript", "tsx", "typescript", "vue"].includes(language);
  const supportsJsx = language === "jsx" || language === "tsx" || language === "vue";
  let cursor = 0;
  let inJsxTag = false;
...
export function SyntaxHighlightedCode({ code, language }: SyntaxHighlightedCodeProps) {
  ...
  return (
    <code class="syntax-highlighted" data-language={normalizedLanguage(language)}>
      {tokens.map((token, index) => token.kind ? (
        <span class={`syntax-${token.kind}`} key={`${index}-${token.kind}`}>{token.text}</span>
      ) : token.text)}
    </code>
  );
}
