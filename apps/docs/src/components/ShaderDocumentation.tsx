...
import { useCallback, useEffect, useMemo, useRef, useState } from "vfx-ui-vue/compat";
import type { CSSProperties, SetStateAction } from "vfx-ui-vue/compat";
...
function escapeTemplateAttribute(value: string) {
  return value
    .replace(/&/g, "&amp;")
    .replace(/"/g, "&quot;")
    .replace(/</g, "&lt;")
...
function formatControlProp(control: ShaderControl, value: boolean | number | string) {
  const prop = vuePropName(control.key);
  if (control.kind === "toggle") return `      :${prop}="${Boolean(value)}"`;
  if (!isRangeControl(control)) {
    return `      ${prop}="${escapeTemplateAttribute(String(value))}"`;
  }
  const numeric = typeof value === "number" ? value : Number(value);
  return `      :${prop}="${numeric.toFixed(control.digits)}"`;
}
...
function generatedExample(shader: ReadyShader, controls: readonly ShaderControl[], settings: PreviewSettings) {
  ...
  if (props) {
    return `<script setup lang="ts">\nimport { ${shader.importName} } from "vfx-ui-vue";\n</script>\n\n<template>\n  <div class="shader-frame">\n    <${shader.importName}\n${props}\n    />\n  </div>\n</template>`;
  }
  return `<script setup lang="ts">\nimport { ${shader.importName} } from "vfx-ui-vue";\n</script>\n\n<template>\n  <div class="shader-frame">\n    <${shader.importName} />\n  </div>\n</template>`;
}
...
function generatedInstallExample(shader: ReadyShader, controls: readonly ShaderControl[], settings: PreviewSettings) {
  ...
  const controlProps = controlPropLines(controls, settings);
  const component = controlProps.length
    ? `    <${shader.importName}\n${controlProps.join("\n")}\n    />`
    : `    <${shader.importName} />`;
...
function buildPromptBundle(shader: ReadyShader, usage: string) {
  const lines = [
    `Add the vfx-ui-vue component "${shader.label}" (${shader.importName}) to this project.`,
    "",
    "Install:",
    "",
...
function buildSkillMarkdown(shader: ReadyShader, usage: string) {
  ...
  const lines = [
    "---",
    `name: use-${slug}`,
    `description: "Use ${shader.label} from vfx-ui-vue: ${shader.description.replace(/"/g, "'")}"`,
    "---",
    "",
    `# Use ${shader.label}`,
    "",
...
function OpenShaderDocumentation({ shader, activeVariantId, onSearchTag, onSelect, onVariantSelect }: ShaderDocumentationProps) {
  ...
  const sourceLanguage = sourceTab === "usage" ? "vue" : sourceTab === "agent" ? "markdown" : "text";
  ...
  return (
    <>
      <div class="pane-inner">
        <main class="doc" id="doc" ref={docRef}>
          <div class="doc-intro">
            <div class="doc-topline">
              <h1>{shader.label}</h1>
              <div class="doc-actions">
                <div class="export-control" ref={exportControlRef}>
                  <button class="export-primary" disabled={exportBusy} onClick={copyPromptBundle}>
                  ...
                  {exportMenuOpen ? (
                    <div class="export-menu card" id="shader-export-menu" role="menu">
            ...
            <div class="tagrow">
              ...
              <span class="metric-tag" title={viewLabel} aria-label={viewLabel}>
                <EyeIcon />{metrics.views.toLocaleString()}
              </span>
              <span class="metric-tag" title={copyLabel} aria-label={copyLabel}>
                <CopyIcon />{metrics.copies.toLocaleString()}
              </span>
            </div>
          </div>

          <section class="demo inset-shadow" id="usage" aria-label="Usage">
            <div class="stage-grid">
              <div
                class={`preview shader-preview ${shader.id} ${shader.category === "Heroes" ? "is-hero-preview" : ""} ${shader.runtime === "dom" ? "is-dom-preview" : ""} ${shader.category === "Footers" ? "is-footer-preview" : ""} ${shader.category === "Glass" || (shader.id === "radiant-dots" || shader.id === "astra-field") ? "is-optical-preview" : ""}`}
              ...
              <Suspense v-slots={{
                default: () => Preview ? (
                  ...
                  <div class="preview-loading" role="status">Renderer not available</div>
                ),
                fallback: () => <div class="preview-loading" role="status">Loading renderer…</div>,
              }} />
            ...
            {shader.variants && shader.variants.length > 1 ? (
              <div class="variant-picker card" aria-label={`${shader.label} variants`}>
                <div class="variant-picker-head">{shader.variants.length} variants</div>
                <div
                ...
                >
                  {shader.variants.map((variant) => {
                    ...
                    return (
                      <button
                        type="button"
                        class={`variant-option inset-shadow${selected ? " active" : ""}`}
                        role="radio"
                        ...
                        onClick={(event) => {
                          ...
                          (event.currentTarget as HTMLButtonElement).scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" });
                        }}
                      >
                        <span class="variant-option-thumbnail">
            ...
            <div class="panel card">
              <div class="panel-head">
                ...
                {activeControls.length ? (
                  ...
                  <span class="panel-ready"><span class="status-dot" />READY</span>
                )}
              </div>
              {activeControls.length ? (
                <div class="controls shader-controls">
                  {activeControls.map((control) => {
                    if (control.kind === "toggle") return <label class="control motion-control" key={control.key}>
                      <span>{control.label}</span>
                      <input type="checkbox" role="switch" checked={Boolean(previewSettings[control.key] ?? control.default)} onInput={(event) => setPreviewSettings((current) => ({ ...current, [control.key]: (event.currentTarget as HTMLInputElement).checked }))} />
                    </label>;
                    ...
                    if (control.kind === "color") {
                      ...
                      const id = `${shader.id}-${activeVariant?.id ?? "default"}-${control.key}`;
                      return (
                        <div class="control color-control inset-shadow" key={control.key}>
                          <label for={id}>{control.label}</label>
                          <div class="color-control-value">
                            <input
                              id={id}
                    ...
                    if (isChoiceControl(control)) {
                      const value = String(previewSettings[control.key] ?? control.default);
                      return (
                        <div class="control choice-control inset-shadow" key={control.key}>
                          <span class="choice-label">{control.label}</span>
                          <div class="choice-options" role="group" aria-label={control.label}>
                    ...
                    if (control.kind === "text") {
                      const value = String(previewSettings[control.key] ?? control.default);
                      return (
                        <div class="control text-control inset-shadow" key={control.key}>
                          <label for={`${shader.id}-${activeVariant?.id ?? "default"}-${control.key}`}>{control.label}</label>
                          <input
                    ...
                    return (
                      <div
                        class="control slider-control inset-shadow"
                        style={sliderStyle}
                        key={control.key}
                        onPointerdown={(event) => {
                        ...
                        onPointerup={(event) => (event.currentTarget as HTMLDivElement).releasePointerCapture(event.pointerId)}
                      >
                        <span class="slider-fill card"><span class="slider-knob" /></span>
                        <label for={`${shader.id}-${control.key}`}>{control.label}</label>
                        <span class="slider-value">{value.toFixed(control.digits)}</span>
                        <input
              ...
              <div class="controls renderer-facts" aria-label="Renderer facts">
                <div class="control inset-shadow">
                ...
                <div class="control inset-shadow">
                  <span>Package</span><strong>vfx-ui-vue</strong>
                </div>
              </div>
            </div>
            </div>

            <div class="code-card card source-card">
              <div class="tabbar">
                <div class="tabs" role="tablist" aria-label="Source details">
                ...
                {sourceTab === "usage" && settingsDirty ? (
                  <span class="live-params-badge" title="Code reflects the current params panel values">Live params</span>
                ) : null}
                <button
                  class="icon-btn inset-shadow"
              ...
              {sourceTab === "agent" ? (
                <div class="source-filebar skill-filebar">
                  <span class="source-file-name">agent notes</span>
              ...
              <pre class={`code source-code${sourceTab === "agent" ? " skill-source" : ""}`}>
          ...
          <div class="table-wrap card">
            <table>
              <thead><tr><th>Prop</th><th>Type</th><th class="col-default">Value</th></tr></thead>
              <tbody>
          ...
          <div class="code-card card source-card">
            <div class="tabbar">
              <div class="tabs" role="tablist" aria-label="Agent notes">
                <span class="tab" role="tab" aria-selected="true">Agent Notes</span>
              </div>
              <button
                class="icon-btn inset-shadow"
            ...
            <pre class="code source-code skill-source">
          ...
          <nav class="pager" aria-label="Component pagination">
            {previous ? (
              <button class="card" onClick={() => onSelect(previous.id)}>
                <span class="k">Previous</span><span class="v">{previous.label}</span>
              </button>
            ) : <span />}
            {next ? (
              <button class="card next" onClick={() => onSelect(next.id)}>
                <span class="k">Next</span><span class="v">{next.label}</span>
              </button>
            ) : <span />}
          </nav>
        </main>
        ...
        <aside class="rail">
          ...
          <nav
            class="toc is-animated"
            aria-label="On this page"
            style={{ "--toc-active-index": activeTocIndex } as CSSProperties}
          >
            <span class="toc-active-line" aria-hidden="true" />
            <span class="toc-active-dot" aria-hidden="true" />
            {TOC_ITEMS.map(({ id, label }) => (
              <div class={`toc-item${activeTocSection === id ? " on" : ""}`} key={id}>
