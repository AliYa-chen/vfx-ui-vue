import { createContext, useContext, useEffect, useRef, useState, type RefObject } from "./vueCompat";
...
export function usePointerUniforms<T extends HTMLElement>(options?: {
  ...
  ease?: number;
}): [RefObject<T>, PointerUniform, boolean, PointerVelocity] {
  ...
  useEffect(() => {
    ...
    // Position as last committed to Vue; the per-frame sweep is the delta
    // the next tick adds to it. Kept off-state so tick stays side-effect
    // free inside the updater.
