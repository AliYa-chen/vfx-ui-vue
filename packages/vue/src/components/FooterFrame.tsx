...
export function FooterFrame({
  ...
  const navigation = groups.length > 0 && (
    <nav class="vfx-footer-nav" aria-label="Footer navigation">
      {groups.map((group, i) => (
        <div class="vfx-footer-group" key={`${group.label}-${i}`}>
          <h3>{group.label}</h3>
  ...
  return (
    <footer
      ref={surfaceRef}
      class={`vfx-footer vfx-footer-${kind} ${className}`}
      style={style}
    ...
    >
      ...
      <div class="vfx-footer-content">
        {content ?? (
          <>
            <div class="vfx-footer-intro">
              {title != null && <h2 class="vfx-footer-title">{title}</h2>}
              {description != null && (
                <div class="vfx-footer-description">{description}</div>
              )}
              {cta && (
                <a class="vfx-footer-cta" href={cta.href}>
                  {cta.label}
                  <FooterArrow />
                </a>
              )}
      ...
      {!before && artwork}
      {kind === "phosphor" && content == null && navigation}
      {(copyright != null || legal.length > 0) && (
        <div class="vfx-footer-legal">
          <span>{copyright}</span>
          {legal.length > 0 && (
