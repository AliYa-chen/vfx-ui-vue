import { childrenFromSlots, useRef, type CSSProperties, type VueNode, type MouseEventHandler } from "../vueCompat";
import { PointerSurfaceContext } from "../usePointerUniforms";

/**
 * Shared layout shell for every vfx-ui-vue hero component.
 *
...
function HeroAction({ action, variant }: { action: HeroCta; variant: string }) {
  ...
  return action.href ? <a class={className} href={action.href}>{action.label}</a>
    : <button type="button" class={className} onClick={action.onClick}>{action.label}</button>;
}
...
export interface HeroShellProps {
  ...
  /** The GPU background layer — rendered in an aria-hidden, pointer-free prop. */
  background: VueNode;
...
export function HeroShell({
  ...
  return (
    ...
    <section
      ref={surfaceRef}
      class={`vfx-hero${className ? ` ${className}` : ""}`}
      data-layout={layout}
      data-scheme={scheme}
      style={{ ...(accent ? ({ "--hero-accent": accent } as CSSProperties) : null), ...style }}
    >
      <style>{HERO_CSS}</style>
      <div class="vfx-hero-bg" aria-hidden="true" style={{ pointerEvents: "none" }}>
        {background}
      </div>
      <div class="vfx-hero-scrim" aria-hidden="true" />
      <div class="vfx-hero-inner vfx-hero-anim">
        {content ?? <>
        {eyebrow ? <p class="vfx-hero-eyebrow">{eyebrow}</p> : null}
        {title != null && <h1 class="vfx-hero-title">{title}</h1>}
        {subtitle ? <p class="vfx-hero-subtitle">{subtitle}</p> : null}
        {primaryCta || secondaryCta ? (
          <div class="vfx-hero-actions">
            {primaryCta ? (
        ...
        {badges?.length ? (
          <div class="vfx-hero-badges">
            {badges.map((badge) => (
              <span class="vfx-hero-badge" key={badge}>{badge}</span>
            ))}
          </div>
        ) : null}
        </>}
