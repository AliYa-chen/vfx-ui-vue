...
/** Complete MIT Vercel optical pipeline, with a Vue-owned lifecycle. */
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (!last || now - last > 1000 / 45) {
        ...
        try {
          ...
          console.error("[vfx-ui-vue] prism", error);
          setFailed(true);
          cleanup();
          return;
        }
    ...
    void (async () => {
      ...
      try {
        ...
        if (!disposed) {
          setFailed(true);
        console.error("[vfx-ui-vue] prism initialization", error);
        }
        cleanup();
      }
    })().catch((error) => {
