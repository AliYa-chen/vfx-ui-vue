import { childrenFromSlots, type CSSProperties } from "../vueCompat";
...
export function FooterVinyl({ brand = "SIDE B", title = "Good things\nstay on repeat.", color = "#252a20", background = "#e8a0ae", labelColor = "#ef623b", interactive = true, style, ...content }: FooterVinylProps) {
  ...
  return <FooterFrame {...content} children={childrenFromSlots(content.children)} brand={brand} title={title} kind="vinyl" before surfaceRef={surface}
    style={{ "--vf-bg": background, "--vf-ink": color, "--vf-accent": color, "--vinyl-label": labelColor, ...style } as CSSProperties}
    artwork={<><style>{CSS}</style><div class="vfx-footer-art vfx-vinyl-art" aria-hidden="true">
      <div class="vfx-vinyl-heading"><span class="vfx-vinyl-kicker">THE END IS A BEGINNING</span><span class="vfx-vinyl-brand">{brand}</span><span class="vfx-vinyl-edition">LONG PLAY / GOOD COMPANY</span></div>
      <div class="vfx-vinyl-record"><div class="vfx-vinyl-label"><strong>{brand}</strong><span class="vfx-vinyl-hole"/><small>SIDE B · 33⅓ RPM</small></div></div>
    </div></>}/>
}
