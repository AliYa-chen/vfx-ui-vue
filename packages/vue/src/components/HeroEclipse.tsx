import { childrenFromSlots, useId, type CSSProperties } from "../vueCompat";
...
export function HeroEclipse({
  ...
  return <StudioHeroFrame {...content} children={childrenFromSlots(content.children)} eyebrow={eyebrow} title={title} subtitle={subtitle} kind="eclipse" surfaceRef={surface}
    style={{ "--studio-paper": background, "--studio-ink": "#f2e9da", "--studio-accent": color, ...style } as CSSProperties}
    artwork={<><style>{CSS}</style><div class="vfx-eclipse-instrument"><svg viewBox="0 0 700 700" fill="none">
      <defs>
        <radialGradient id={`${id}-sun`}><stop stop-color="#fff3d0"/><stop offset=".75" stop-color={color}/><stop offset="1" stop-color={color} stop-opacity=".6"/></radialGradient>
        <radialGradient id={`${id}-halo`}><stop offset=".45" stop-color={color} stop-opacity=".2"/><stop offset="1" stop-color={color} stop-opacity="0"/></radialGradient>
      </defs>
      <circle cx="350" cy="350" r="300" fill={`url(#${id}-halo)`}/>
      {[300, 282, 246, 217].map((r, i) => <circle key={r} cx="350" cy="350" r={r} stroke={color} stroke-opacity={i === 1 ? .55 : .23} stroke-width=".7"/>)}
      {Array.from({ length: 120 }, (_, i) => <path key={i} d={`M350 68v${i % 10 === 0 ? 19 : i % 5 === 0 ? 12 : 5}`} transform={`rotate(${i * 3} 350 350)`} stroke={color} stroke-opacity={i % 5 === 0 ? .85 : .35} stroke-width={i % 10 === 0 ? 1.4 : .7}/>)}
      {Array.from({ length: 12 }, (_, i) => { const a = (i * 30 - 90) * Math.PI / 180; return <text key={i} x={350 + Math.cos(a) * 260} y={354 + Math.sin(a) * 260} text-anchor="middle" fill={color} font-family="monospace" font-size="9" opacity=".8">{String(i * 30).padStart(3, "0")}</text>; })}
      <path d="M25 350h650M350 25v650" stroke={color} stroke-opacity=".16" stroke-dasharray="3 7"/>
      <circle cx="350" cy="350" r="174" fill={`url(#${id}-sun)`}/>
      <circle class="vfx-eclipse-disc" cx="350" cy="350" r="163" fill={background} stroke={color} stroke-width=".6"/>
      <g class="vfx-eclipse-hands" stroke={color}><path d="M350 47v72M343 54l7-7 7 7M350 581v72"/><circle cx="350" cy="138" r="4" fill={color}/><path d="M343 350h14m-7-7v14" stroke-opacity=".6"/></g>
      <text x="350" y="685" text-anchor="middle" fill={color} font-size="9" font-family="monospace" letter-spacing="4">S O L  /  L U N A</text>
    </svg></div></>}/>
}
