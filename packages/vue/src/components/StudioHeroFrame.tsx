import { childrenFromSlots, type CSSProperties, type VueNode, type RefObject } from "../vueCompat";
...
export function StudioHeroFrame({ eyebrow, title, subtitle, primaryCta, secondaryCta, children, className = "", style, artwork, surfaceRef, kind }: HeroContentProps & {
  ...
  return <section ref={surfaceRef} class={`vfx-studio-hero vfx-studio-${kind} ${className}`} style={style}>
    <style>{CSS}</style>
    <div class="vfx-studio-art" aria-hidden="true">{artwork}</div>
    <div class="vfx-studio-copy">{content ?? <>
      {eyebrow && <p class="vfx-studio-eyebrow">{eyebrow}</p>}
      {title != null && <h1 class="vfx-studio-title">{title}</h1>}
      {subtitle && <p class="vfx-studio-subtitle">{subtitle}</p>}
      {actions.length > 0 && <div class="vfx-studio-actions">{actions.map((action, index) => action.href
        ? <a class="vfx-studio-action" key={index} href={action.href}>{action.label}</a>
        : <button class="vfx-studio-action" key={index} type="button" onClick={action.onClick}>{action.label}</button>)}</div>}
    </>}</div>
  </section>;
}
