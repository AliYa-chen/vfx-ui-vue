import { childrenFromSlots, useMemo, type CSSProperties } from "../vueCompat";
...
export function HeroContour({
  ...
  return <StudioHeroFrame {...content} children={childrenFromSlots(content.children)} eyebrow={eyebrow} title={title} subtitle={subtitle} kind="contour" surfaceRef={surface}
    style={{ "--studio-paper": background, "--studio-ink": "#27362a", "--studio-accent": color, ...style } as CSSProperties}
    artwork={<><style>{CSS}</style><div class="vfx-contour-sun"/><div class="vfx-contour-landscape"><svg viewBox="0 0 900 800" preserveAspectRatio="xMidYMax slice" fill="none">
      {Array.from({ length: 8 }, (_, band) => <g data-contour-band key={band}>{paths.slice(band * 6, band * 6 + 6).map((d, i) => <path key={i} d={d} fill={background} stroke={(band * 6 + i) % 8 === 0 ? color : "#53634f"} stroke-width={(band * 6 + i) % 8 === 0 ? 1.8 : .8}/>)}</g>)}
    </svg></div></>}/>
}
