...
const SHARED_FILES = [
  { path: "vfx/vueCompat.ts", from: join(vueSrc, "vueCompat.ts") },
  { path: "vfx/VfxCanvas.tsx", from: join(vueSrc, "VfxCanvas.tsx") },
  { path: "vfx/color.ts", from: join(vueSrc, "utils/color.ts") },
  { path: "vfx/usePointerUniforms.tsx", from: join(vueSrc, "usePointerUniforms.ts") },
];
...
function rewriteImports(content, depNames, inVfx) {
  ...
  let out = content
    ...
    .replace(/from "\.\.\/vueCompat"/g, `from "${p}vueCompat"`)
...
function componentCode(component) {
  const p = join(vueSrc, "components", `${component}.tsx`);
...
function buildItem(entry) {
  ...
  const item = {
    ...
    dependencies: entry.sharedFiles === false
      ? ["vue@^3.5.0", ...(entry.npmDependencies ?? [])]
      : ["vue@^3.5.0", "vgpu@0.3.1", ...(entry.npmDependencies ?? [])],
    categories: entry.categories,
    docs: entry.description,
    files: [
      ...(entry.sharedFiles === false
        ? [{
            path: "vfx/vueCompat.ts",
            type: "registry:component",
            content: read(join(vueSrc, "vueCompat.ts")),
            target: "components/vfx/vueCompat.ts",
          }]
        : SHARED_FILES.map((f) => ({
            path: f.path,
            type: "registry:component",
      ...
      ...(entry.motion ? [{ path: "vfx/usePointerMotion.ts", type: "registry:component", content: read(join(vueSrc, "usePointerMotion.ts")), target: "components/vfx/usePointerMotion.ts" }] : []),
      ...(entry.sharedFiles === false && deps.includes("HeroShell") ? [{ path: "vfx/usePointerUniforms.tsx", type: "registry:component", content: read(join(vueSrc, "usePointerUniforms.ts")), target: "components/vfx/usePointerUniforms.tsx" }] : []),
      ...depFiles,
      {
