...
const vueRequire = createRequire(root + "/packages/vue/package.json");
const vgpuRequire = createRequire(vueRequire.resolve("vgpu"));
...
await fs.writeFile(
  root + "/packages/vue/src/components/PrismEngine.tsx",
  "// @ts-nocheck\n/* Vercel VGPU prism light pipeline.\n" +
    (await fs.readFile(root + "/references/vgpu/LICENSE", "utf8")) +
    "\n*/\n" +
    result.outputFiles[0].text
      // WGSL loader diagnostics include absolute source paths in comments.
      // Keep generated sources reproducible and avoid publishing build-machine paths.
      .replaceAll(root + "/", "<vfx-ui-vue-source>/")
      .replace(
