...
const SHADER_IMPORTS = {
  ...
  // hero-globe is excluded: its visual uses cobe (WebGL),
...
function makeEntrySource(requested) {
  ...
  const imports = [
    ...new Set(shaderNames.map((n) => { const [sym, p] = SHADER_IMPORTS[n]; return `import { ${sym} } from "${join(root, "packages/vue/src/components", p.slice(2))}";` })),
    ...new Set(pipelineNames.map((n) => { const p = PIPELINES[n]; return `import { ${p.fn} } from "${join(root, "packages/vue/src/components", p.module.slice(2))}";` })),
  ].join("\n");
